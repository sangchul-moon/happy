...
export function useCLIDetection(machineId: string | null): CLIAvailability {
    ...
    useEffect(() => {
        ...
        const detectCLIs = async () => {
            ...
            log.debug('[useCLIDetection] Starting detection for machineId:', machineId);
            ...
            try {
                ...
                if (cancelled) return;
                log.debug('[useCLIDetection] Result:', { success: result.success, exitCode: result.exitCode, stdout: result.stdout, stderr: result.stderr });
                ...
                if (result.success && result.exitCode === 0) {
                    ...
                    log.debug('[useCLIDetection] Parsed CLI status:', cliStatus);
                    setAvailability({
                    ...
                    log.debug('[useCLIDetection] Detection failed (success=false or exitCode!=0):', result);
                    setAvailability({
                        claude: null,
                        codex: null,
                        gemini: null,
                ...
                log.debug('[useCLIDetection] Network/RPC error:', error);
                setAvailability({
                    claude: null,
                    codex: null,
                    gemini: null,
